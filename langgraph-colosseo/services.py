...
from graphs import TranslationGraph, AnalysisGraph
...
class TranslationService:
    def __init__(self, ollama_client: OllamaClient):
        ...
        self.translation_graph = TranslationGraph(ollama_client)
    ...
    async def translate_text(self, request: TranslationRequest) -> TranslationResult:
        """Translate text using the translation graph"""
        ...
        try:
            ...
            translated_text = await asyncio.get_event_loop().run_in_executor(
                None,
                self.translation_graph.translate,
                request.text,
...
class AnalysisService:
    def __init__(self, ollama_client: OllamaClient):
        self.ollama_client = ollama_client
        self.analysis_graph = AnalysisGraph(ollama_client)
    ...
    async def analyze_grammar(self, request: AnalysisRequest) -> AnalysisResult:
        """Analyze grammar using the analysis graph"""
        validation_error = self.validate_request(request)
        if validation_error:
